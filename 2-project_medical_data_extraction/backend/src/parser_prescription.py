...
class PrescriptionParser(MedicalDocParser):
    def __init__(self,text):
        MedicalDocParser.__init__(self, text)                    # calling init of the parent class
    ...
    def get_field(self, field_name):
        ...
        if pattern_object:
            matches = re.findall(pattern_object['pattern'], self.text, flags=pattern_object['flags'])     # In flags parameter if you not specifying anything, then you should specify `0` not `None`, actually i was doing that so it was returning me error.
            if len(matches) > 0:
